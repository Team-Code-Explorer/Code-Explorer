...
// let studentPhotoInput = document.getElementById('studentPhoto');
...
function handleEnrollmentSubmission(evt) {
    ...
    if (fname && lname && gender && age && dob && email && phoneCode && phoneNumber && address && state && pincode && hobbies.length > 0  && termsAgreed) {
        // Push form data to the "cProgrammingEnrollment" node in the database
        push(ref(db, 'cProgrammingEnrollment'), {
            firstName: fname,
            lastName: lname,
            gender: gender,
            age: age,
